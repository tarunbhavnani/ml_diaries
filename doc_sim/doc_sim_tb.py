...
tok= Tokenizer(split=" ", oov_token= '-OOV-')
...
word_vecs=np.zeros((tokenized_padded_document.shape[0], tokenized_padded_document.shape[1],50 ))

for num,docu in enumerate(tokenized_padded_document):
    print(docu)
    for num_vec,vec in enumerate(docu):
        print(vec)
        if vec in vocab_inv and vocab_inv[vec] in embeddings_index:
            word_vecs[num][num_vec]= embeddings_index[vocab_inv[vec]]
        else:
            word_vecs[num][num_vec]= np.zeros(50)
...
for num,docu in enumerate(tokenized_padded_document):
    ...
    for num_vec,vec in enumerate(docu):
        #print(vec)
        if vec in vocab_inv and vocab_inv[vec] in embeddings_index and vocab_inv[vec] in list(df_tfidf):
            tfidf_multiple= df_tfidf.iloc[num][vocab_inv[vec]]
            word_vecs[num][num_vec]= embeddings_index[vocab_inv[vec]]*tfidf_multiple
...
# =============================================================================
# now we can add(or average)
# =============================================================================

final_embeddings= np.zeros((word_vecs.shape[0], word_vecs.shape[2]))
for num,docu in enumerate(word_vecs):
    #pass
    for num_vec, vec in enumerate(docu):
        #pass
        final_embeddings[num]+=vec
