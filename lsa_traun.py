...
import os
os.chdir('/home/tarun/Desktop/LSA_Classification-master')
###############################################################################
...
#at 50 features: 93.82, alrady over overall.
#at 50 features: 93.91, marginal increment over 50.
#at 50 features: 93.95, marginal increment over 100.


#we see that it is not increasing. lets find out till when does it increase.
feat=[]
varexp=[]
acc=[]
...
for i in  range(1,5,1):
    ...
    varexp.append(explained_variance)
    ...
    for i in range(0,len(p)):
        if p[i] == y_test[i]:
            numRight += 1
    ac=float(numRight) / float(len(y_test)) * 100.0
    print(ac)
    acc.append(ac)

import pandas a spd
df=pd.DataFrame(list(zip(feat,varexp,acc)), columns=["feat","varexp","acc"])
...
#elbow at ~20
#we take 20 svds to define our data
