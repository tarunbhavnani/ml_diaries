# -*- coding: utf-8 -*-
"""
Created on Sat Jun 12 13:32:30 2021

@author: ELECTROBOT
"""

#percentage of dale difficult words
#percentage of nouns
#number of syllabes/hardwords cmudict
...
#num words
train['num_words']= train['excerpt'].apply(lambda x: len(x.split()))
#num unique words
train['num_unique_words']= train['excerpt'].apply(lambda x: len(set(x.split())))
...
train['avg_sent_len']= train['excerpt'].apply(lambda x: sent_len(x))
...
train['dale_words']= train['excerpt'].apply(lambda x: len([i for i in x.split() if i not in dale_chall_list]))
...
train['hard_word']= train['excerpt'].apply(lambda x: hard_word(x))    
    
# =============================================================================
#avg sylabble count

def phenomes_score(text):
    doc= nlp(text)
    sents= doc.sents
    total_score=[]
    for sent in sents:
        ...
        for i in text2.split(' ') :
            try:
                ...
                ns=nsyl(i.lower())
                if type(ns)==int :
                    total_score.append(ns)
                else:
                    total_score.append(ns[0])
                    
            except:
                pass
    return sum(total_score)/len(total_score)

train['phenome_score']= train['excerpt'].apply(lambda x: phenomes_score(x))    
...
train['spacy_trial']= train['excerpt'].apply(lambda x :spacy_trial(x))
...
X=train[['standard_error','num_words','num_unique_words','avg_sent_len','dale_words','hard_word','phenome_score','spacy_trial']].values
...
# =============================================================================
# use hidden layers to get the proper connect with the features listed
# =============================================================================
