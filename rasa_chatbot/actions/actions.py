...
class Actioninterviewstart(Action):
    ...
    def run(self, dispatcher, tracker, domain):
      ...
      if (user_name=="Dear" and user_cell=="none"):
           ...
           dispatcher.utter_message("PD शुरू करने के लिए कृपया Ref आईडी इनपुट करें अन्यथा 12345 इनपुट करें।")
    ...
    def run(self, dispatcher, tracker, domain):
        ...
        try:
         ...
         if n>1:
                    ...
                    #user_name=str(df[df.applicant_1_phone==int(user_cell)].last_name.item())  
                    user_name=str(json_response["applicant_1_first_name"])  
                    dispatcher.utter_message("नमस्ते {}, अब हम PD शुरू करेंगे!".format(user_name))
                    #return[SlotSet('user_name', user_name),SlotSet('user_cell', user_cell)]
                    #from rasa_core_sdk.events import FollowupAction
...
class ActionPrivate(Action):
    ...
    def run(self, dispatcher, tracker, domain):
      ...
      try:
        directors= str(json_response["directors"])
      except:
        directors=""
...
class ActionBusinessYearsExplain(Action):
    ...
    def run(self, dispatcher, tracker, domain):
        ...
        #age= int(df[df.applicant_1_phone==int(user_cell)].age.item())  
        try:
          age=now.year-int(re.findall('\d+', json_response["applicant_1_dob"])[0])
        except:
          age=0#############################Default case here
          #####################################
          #####################################
          #####################################3
        last_message= tracker.latest_message['text']
...
class ActionMonthlySales(Action):
    ...
    def run(self, dispatcher, tracker, domain):
      ...
      #ActionSave.run('action_save',dispatcher, tracker, domain)
      counter="action_monthly_sales2"
      current="action_monthly_sales"
      return [SlotSet('counter', counter),FollowupAction("action_listen"),SlotSet('current', current) ]

class ActionMonthlySales2(Action):
    
    def name(self):
        return "action_monthly_sales2"
    def run(self, dispatcher, tracker, domain):
      #dispatcher.utter_message("Is your business seasonal? If yes explain.")
      dispatcher.utter_message("क्या आपका व्यवसाय सीजनल है? यदि हाँ तो समझाइए।")
      #ActionSave.run('action_save',dispatcher, tracker, domain)
      counter="action_turnover"
      current="action_monthly_sales2"
      return [SlotSet('counter', counter),FollowupAction("action_listen"),SlotSet('current', current) ]
...
class ActionTurnover(Action):
    ...
    def run(self, dispatcher, tracker, domain):
      ...
      #ActionSave.run('action_save',dispatcher, tracker, domain)
      counter="action_turnover2"
      current="action_turnover"
      return [SlotSet('counter', counter),FollowupAction("action_listen"),SlotSet('current', current) ]

class ActionTurnover2(Action):
    
    def name(self):
        return "action_turnover2"
    def run(self, dispatcher, tracker, domain):
      #dispatcher.utter_message("What are your expansion plans to increase the turnover/ margin?")
      dispatcher.utter_message("टर्नओवर / मार्जिन बढ़ाने के लिए आपकी विस्तार योजनाएं क्या हैं?")
      #ActionSave.run('action_save',dispatcher, tracker, domain)
      counter="action_cash"
      current="action_turnover2"
      return [SlotSet('counter', counter),FollowupAction("action_listen"),SlotSet('current', current) ]
...
class ActionGstStatus(Action):
    ...
    def run(self, dispatcher, tracker, domain):
      ...
      #ActionSave.run('action_save',dispatcher, tracker, domain)
      counter="action_gst_status2"
      current="action_gst_status"
      return [SlotSet('counter', counter),FollowupAction("action_listen"),SlotSet('current', current) ]
    
class ActionGstStatus2(Action):
    
    def name(self):
        return "action_gst_status2"
    def run(self, dispatcher, tracker, domain):
      
      
      current= "action_gst_status2"
      ...
      if (last_message=="yes"):
        ...
        #dispatcher.utter_message("What is the amount of GST payable due as on date.")
        dispatcher.utter_message("समय पर जीएसटी का भुगतान क्यों नहीं किया गया? gst का कितना भुगतान बाकी है?")
...
class ActionLoanAmount(Action):
    ...
    def run(self, dispatcher, tracker, domain):
      ...
      #ActionSave.run('action_save',dispatcher, tracker, domain)
      counter="action_loan_amount2"
      current="action_loan_amount"
      return [SlotSet('counter', counter),FollowupAction("action_listen"),SlotSet('current', current) ]

class ActionLoanAmount2(Action):
    
    def name(self):
        return "action_loan_amount2"
    def run(self, dispatcher, tracker, domain):
      #dispatcher.utter_message("What is the maximum emi you are looking at?")
      dispatcher.utter_message("आप प्रति माह कितना ईएमआई का भुगतान कर सकते हैं")
      #ActionSave.run('action_save',dispatcher, tracker, domain)
      counter="action_end_use"
      current="action_loan_amount2"
      return [SlotSet('counter', counter),FollowupAction("action_listen"),SlotSet('current', current) ]
...
class ActionFamily3(Action):
    ...
    def run(self, dispatcher, tracker, domain):
      ...
      if (last_message=="self"):
        ...
        dispatcher.utter_message("आप कब से इस घर में रह रहे हैं? इस जगह के लिए आप क्या किराया देते हैं?")
        counter="action_family4"
        return [SlotSet('current', current),SlotSet('counter', counter),FollowupAction("action_listen")]
...
class ActionFamily5(Action):
    ...
    def run(self, dispatcher, tracker, domain):
      dispatcher.utter_message("आपका मूल निवास क्या है?(What is your native place?)")
      #dispatcher.utter_message("Where do you stay?")
      #ActionSave.run('action_save',dispatcher, tracker, domain)
      current="action_family5"
      counter="action_family6"
      return [SlotSet('counter', counter),FollowupAction("action_listen"),SlotSet('current', current) ]

class ActionFamily6(Action):
    
    def name(self):
        return "action_family6"
    def run(self, dispatcher, tracker, domain):
      buttons = [{'title': 'हाँ' , 'payload': 'Yes'}, {'title': 'नहीं', 'payload': 'No'}] 
      dispatcher.utter_button_message("क्या आपके परिवार का कोई और सदस्य है, जो अलग स्रोत से कमाता है? ", buttons)
      #dispatcher.utter_message("Where do you stay?")
      #ActionSave.run('action_save',dispatcher, tracker, domain)
      current="action_family6"
      counter="action_family7"
      return [SlotSet('counter', counter),FollowupAction("action_listen"),SlotSet('current', current) ]
    
class ActionFamily7(Action):
    
    def name(self):
        return "action_family7"
    def run(self, dispatcher, tracker, domain):
      
      
      current= "action_family7"
      counter="action_family8"
      last_intent= tracker.latest_message['intent'].get('name')
      last_message= tracker.latest_message['text']
      if (last_message=="Yes"):
        dispatcher.utter_message("Please elaborate")
...
class ActionFamily8(Action):
    
    def name(self):
        return "action_family8"
    def run(self, dispatcher, tracker, domain):
      
      dispatcher.utter_message("क्या आपके पास कोई current investments या property ownership है?  कृपया विस्तार से बताएं।")
      #dispatcher.utter_message("Where do you stay?")
      #ActionSave.run('action_save',dispatcher, tracker, domain)
      current="action_family8"
      counter="action_ref"
      return [SlotSet('counter', counter),FollowupAction("action_listen"),SlotSet('current', current) ]
...
class ActionRef2(Action):
    ...
    def name(self):
        return "action_ref2"
    def run(self, dispatcher, tracker, domain):
      dispatcher.utter_message("कृपया रेफरेंस चेक के लिए 2 supplier और 2 ग्राहकों का नाम और सेल नंबर प्रदान करें")
      #dispatcher.utter_message("Where do you stay?")
      #ActionSave.run('action_save',dispatcher, tracker, domain)
      current="action_ref2"
      counter="action_ref3"
