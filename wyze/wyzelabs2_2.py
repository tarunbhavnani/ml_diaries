...
for num,user_id in enumerate(ids):
    ...
    x=['_'.join(i.split("_")[:2]) for i in tt.rule]
    all_rules=[i for i in all_rules if '_'.join(i.split("_")[:2])  in x]
...
ss_.to_csv(r"C:\Users\tarun\Desktop\wyzelabs\submisssion_8.csv", index=False)
#score: .126
