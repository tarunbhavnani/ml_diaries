...
# -*- coding: utf-8 -*-
"""
Created on Sun Apr 14 17:55:56 2019
...
data_text=newsgroups_train.data
target= newsgroups_train.target


import pandas as pd
#data = pd.read_csv('abcnews-date-text.csv', error_bad_lines=False);
data_text=pd.DataFrame(data=newsgroups_train.data, columns=["headline_text"])
list(data_text)
data_text.iloc[1]
...
gh=["base","base","html","html","base"]

dictionary.doc2bow(gh)
...
for i in range(len(bow_doc_310)):
    print("Word {} (\"{}\") appears {} time.".format(bow_doc_310[i][0], 
                                               dictionary[bow_doc_310[i][0]], bow_doc_310[i][1]))
...
lda_model_tfidf = gensim.models.LdaMulticore(corpus_tfidf, num_topics=4,
...
lda_model.print_topic(3, 5)#5 is the number of top 5 terms that define the topic
