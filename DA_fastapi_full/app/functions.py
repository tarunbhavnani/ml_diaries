...
class Qnatb(object):
    ...
    def extract_answer_blobs(self, question, responses,num,  model="minilm"):

        # responses= self.get_response_cosine(question)

        final = []
        temp = []
        responses= responses[:num]
        for i in responses:
        ...
        for sentence in final:
            answer, start_logit,_ = self.answer_question(question, sentence)
...
def get_collection(collection, user_id):
    if collection == None or collection == "" or collection=="user" or collection=="myuploads":
        collection_var=user_id
    else:
        collection_var="coll_"+str(collection)

    return collection_var


def process_upload_files(files, collection, user_id):
    collection_var= get_collection(collection,user_id)
    for file in files:
        if file and allowed_file(file):
            try:
                obj= file.file.read()
                file_path= collection_var+"_"+file.filename
                cache.write_to_cache_user(file_path, obj)
...
def get_final_responses(qna, question, collection, user_id):
    collection_var=get_collection(collection, user_id)
    fp_name= collection_var+"_fp"
    obj= cache.read_from_cache_user(fp_name)
    fp= pickle.loads(obj)
    response_sents= fp.get_response_cosine(question)
    results= qna.extract_answer_blobs(question, response_sents, 50)
    results= [i for i in results if i["doc"]!=""]
    if results!=[]:
        final={results:results, "error": "None", "status": "success"}
    else:
...
def get_redis_keys(collection, user_id):
    ...
    names= cache.keys()
    names= [name.decode() for name in names]
    if collection=="all_users":
        return [n for n in names if n.split("_")[0]!="coll"]
    elif collection=="all_collection":
        return [n for n in names if n.split("_")[0]=="coll"]
    elif collection=="all_collection_fp":
        ret= [n[5:-3] for n in names if n.split("_")[0]=="coll" and n[-2:]=="fp"]
        ret.append("myuploads")
        return ret
    elif collection =="everything":
        return [n for n in names]

    else:
        return [n for n in names if n[:len(collection_var)]==collection_var]

def delete_files(collection, user_id):
    names= get_redis_keys(collection=collection, user_id=user_id)
    for name in names:
        cache.delete(name)
...
def transfer_fp(collection, user_id):
    try:
        fp_name= user_id+"_fp"
        obj= cache.read_from_cache_user(fp_name)
