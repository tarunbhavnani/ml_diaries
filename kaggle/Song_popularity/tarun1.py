...
preds=[0 if i>thresh else 1 for i in pr]
...
pr=[i[0] for i in lr.predict_proba(jk)]
sns.kdeplot(pr)
thresh=np.median(pr)
...
for i in cat:
    #break
    tr[i]= tr[i].astype('category')
tr_cat=pd.get_dummies(tr[cat])


ss= StandardScaler()
tr_numr=pd.DataFrame(ss.fit_transform(tr[numr]))
tr_numr.columns= list(tr[numr])


tr_final=pd.concat([tr_numr,tr_cat], axis=1)

lr= LogisticRegression()

lr.fit(tr_final, labels)

preds=lr.predict(tr_final)

pr=[i[0] for i in lr.predict_proba(tr_final)]
sns.kdeplot(pr)
thresh=np.median(pr)
