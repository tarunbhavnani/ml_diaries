...
def my_form_post():
    ...
    prediction = predict(str(text))

    return render_template('form.html', result=prediction)


#define upload model if has to be uploaded from local
...
def predict(sentence):
    if model:
        encoded = tok.encode_plus(sentence, padding="max_length", max_length=512, truncation=True, return_attention_mask=True)
